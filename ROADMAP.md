...
- [?] Is the loader sourced by the agent process? (`!echo $AGENT_DIR $KUBECONFIG` in Claude Code after a restart)
- [?] The LAN IP in the project kit: the allow was added from the TUI. Bare vs `/32` vs policy level, check with `sbx policy ls`

### Remaining providers [ ] (only those actually in use)
| Provider | Identity | Delivery | Status |
|---|---|---|---|
| Azure | SP `Reader` scoped to the subscription | `.env` + `az login --service-principal` in `post-create.sh` | to do, priority (infra-azure repo) |
| Hetzner | **Read** API token | `sbx secret set-custom` (bearer, proxy) | to do, validates proxy injection |
| Scaleway | `AllProductsReadOnly` policy | `sbx secret` (`X-Auth-Token`) | to do |
| ArgoCD | account/role `get` only | `sbx secret` (bearer) + host in project kit | to do |
| Flux | nothing: goes through the kubeconfig | n/a | done de facto |
| AWS | `ViewOnlyAccess` role | `.env.session` (STS 1h, step 4) | to do |
| GCP | SA `roles/viewer` | `.env.session` (1h token, step 4) | to do |
| OVH | app keys `GET /*` | `.env` | to do |
| Terraform/OpenTofu | n/a | state decision: RO backend (secrets in clear) / redacted state in `shared/` / `plan` in CI | decision pending |
...
- [ ] `harness env`: generates `data/agent/.env.session` (AWS STS creds, `gcloud auth print-access-token`), 1 h lifetime
- [ ] `harness run` calls `env` before attaching if ephemeral providers are configured
- [ ] Rotation: `.env.session` never edited by hand, re-read at the next session without `--fresh`

The mechanism already exists (`.env` mounted `:ro`, loader); only the generation script is missing.

---

## Step 5. Headroom, measured [ ]

- [ ] Baseline: one week of sessions with adapted effort/model (Sonnet for execution, Opus for planning, recaps cut, unused MCPs disabled with `/mcp`)
...
- [ ] Document the harness (README) so it can be picked up again in six months
